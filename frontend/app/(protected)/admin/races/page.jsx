...
export default function RacesPage() {
  ...
  const [selectedTypeFilter, setSelectedTypeFilter] = useState('all'); // all, single, multi
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    // 1.5 Filter by race type
    if (selectedTypeFilter !== 'all') {
      const isMulti = selectedTypeFilter === 'multi';
      result = result.filter((r) => !!r.is_multi_race === isMulti);
    }
  ...
  }, [racesList, searchQuery, sortBy, selectedTypeFilter]);
    ...
    <Container className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 bg-muted/5 p-4 rounded-xl border border-border/80 shadow-xs">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 flex-1">
          ...
          {/* Type Filter */}
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-muted-foreground shrink-0">Yarış Tipi:</span>
            <Select value={selectedTypeFilter} onValueChange={setSelectedTypeFilter}>
              ...
              <SelectContent className="bg-card border border-border">
                <SelectItem value="all">Tümü</SelectItem>
                <SelectItem value="single">Tekil Yarış</SelectItem>
                <SelectItem value="multi">Çoklu Yarış</SelectItem>
              </SelectContent>
            </Select>
          </div>
          
          {/* Reset Filters Link */}
          {(searchQuery || selectedCategoryFilter !== 'all' || selectedStatusFilter !== 'all' || selectedTypeFilter !== 'all') && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => {
