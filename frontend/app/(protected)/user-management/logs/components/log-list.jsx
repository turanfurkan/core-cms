...
const LogList = () => {
  ...
  const fetchLogs = async ({ pageIndex, pageSize, searchQuery, selectedLogName, selectedEvent }) => {
    ...
    const response = await apiFetch(`/api/user-management/logs?${params.toString()}`);
    if (!response.ok) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['user-activity-logs', pagination, searchQuery, selectedLogName, selectedEvent],
    queryFn: () =>
      fetchLogs({
  ...
  const columns = useMemo(
    () => [
            ...
            return (
              <div className="flex items-center gap-2">
                <Avatar className="size-8">
                  <AvatarFallback className="bg-muted text-muted-foreground text-xs font-semibold">SYS</AvatarFallback>
                </Avatar>
                <div className="space-y-px">
                  <div className="font-semibold text-sm text-foreground">System</div>
                  <div className="text-muted-foreground text-xs">Automated Process</div>
                </div>
              </div>
            );
          }
          const initials = getInitials(causer.name || causer.email);
          return (
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-px">
                <div className="font-semibold text-sm text-foreground">{causer.name}</div>
                <div className="text-muted-foreground text-xs">{causer.email}</div>
              </div>
            </div>
          );
        },
      ...
      },
      {
        ...
        id: 'log_name',
        header: ({ column }) => (
          <DataGridColumnHeader title="Log Type" visibility={true} column={column} />
        ),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        id: 'event',
        header: ({ column }) => (
          <DataGridColumnHeader title="Action / Event" visibility={true} column={column} />
        ),
        cell: ({ row }) => {
      ...
      },
      {
        ...
        id: 'created_at',
        header: ({ column }) => (
          <DataGridColumnHeader title="Timestamp" visibility={true} column={column} />
        ),
        cell: ({ row }) => (
    ...
    return (
      <div className="space-y-2.5">
        <h4 className="font-semibold text-xs text-foreground flex items-center gap-1.5 border-b pb-1">
          <Terminal className="size-3.5 text-primary" /> Audit Logs (Change Diff)
        </h4>
  ...
  // Other dynamic properties component
  const renderExtraMeta = (properties) => {
    ...
    return (
      <div className="space-y-2.5">
        <h4 className="font-semibold text-xs text-foreground flex items-center gap-1.5 border-b pb-1">
          <Info className="size-3.5 text-primary" /> Additional Information
        </h4>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="user.auth">Authentication</SelectItem>
                <SelectItem value="user.profile">User Profile</SelectItem>
                <SelectItem value="content">Content Logs</SelectItem>
                <SelectItem value="media">Media Logs</SelectItem>
              </SelectContent>
            </Select>
            <Select
              onValueChange={(val) => {
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <div className="flex items-center justify-between pr-6">
              <DialogTitle className="flex items-center gap-2">
                <Eye className="size-5 text-primary" /> Log Entry Details
              </DialogTitle>
          ...
          <DialogBody className="space-y-5 py-4 max-h-[70vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-muted/30 border border-border p-4 rounded-xl">
              <div className="space-y-3">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <span className="text-[10px] uppercase font-bold text-muted-foreground block">Causer (User)</span>
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <span className="text-[10px] uppercase font-bold text-muted-foreground block">Timestamp</span>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <span className="text-[10px] uppercase font-bold text-muted-foreground block">IP Address</span>
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <span className="text-[10px] uppercase font-bold text-muted-foreground block">Device / User Agent</span>
            ...
            <div className="space-y-1.5">
              <span className="text-[10px] uppercase font-bold text-muted-foreground block">Action Description</span>
