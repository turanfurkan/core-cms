...
export default function WidgetsPage() {
  ...
  const { data: response, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiFetch('/api/admin/marketing/widgets?limit=100');
      if (!res.ok) throw new Error('Widget bileşenleri yüklenemedi.');
      return res.json();
    },
  });

  const widgetsList = response?.data || [];

  // Filter widgets locally based on search query
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async ({ id, is_active, key, type, config }) => {
      const res = await apiFetch(`/api/admin/marketing/widgets/${id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!res.ok) throw new Error('Widget durumu güncellenemedi.');
      return res.json();
    },
    onSuccess: () => {
      ...
      toast.custom(
        () => (
          <Alert variant="mono" icon="success" close={false}>
            ...
            <AlertTitle>Widget aktiflik durumu güncellendi.</AlertTitle>
          </Alert>
        ),
        { position: 'top-center' }
      );
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      ...
      if (!res.ok) throw new Error('Widget silinemedi.');
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-widgets'] });
      toast.custom(
        () => (
          <Alert variant="mono" icon="success" close={false}>
            ...
            <AlertTitle>Widget başarıyla silindi.</AlertTitle>
          </Alert>
        ),
        { position: 'top-center' }
      );
  ...
  const handleDelete = (id, e) => {
    e.stopPropagation();
    if (confirm('Bu widget bileşenini kalıcı olarak silmek istediğinizden emin misiniz?')) {
  ...
  return (
    <>
      <Container>
        <Toolbar>
          <ToolbarHeading>
            <ToolbarTitle>Widget Bileşenleri Yönetimi (Widgets)</ToolbarTitle>
            <Breadcrumb>
              <BreadcrumbList>
                <BreadcrumbItem>
        ...
        <Card className="flex flex-col md:flex-row items-center gap-4 p-5 mb-6 select-none justify-between">
          <div className="relative w-full md:max-w-xs">
            ...
            <Input
              type="text"
              placeholder="Widget anahtar veya tür ara..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="ps-9 pe-9 h-10 w-full font-mono"
            />
            {searchQuery && (
              <Button
                mode="icon"
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-2 select-none">
            ...
            <span className="text-xs text-muted-foreground">Widget'lar listeleniyor...</span>
          </div>
        ) : (
          <Card>
            <CardTable>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-border bg-muted/10 text-muted-foreground text-xs font-bold uppercase tracking-wider select-none">
                      <th className="py-3.5 px-5">Bileşen Anahtarı (Key)</th>
                      <th className="py-3.5 px-4">Bileşen Türü (Type)</th>
                  ...
                  <tbody className="divide-y divide-border text-sm">
                    {filteredWidgets.map((item) => (
                      ...
                      >
                        <td className="py-3.5 px-5 font-mono text-xs font-bold text-foreground text-primary">
                          {item.key}
                        </td>
            ...
            {filteredWidgets.length === 0 && (
              <div className="flex flex-col items-center justify-center py-20 border-t border-border bg-card rounded-b-xl select-none">
                <Layout className="size-10 text-muted-foreground/60 mb-2.5" />
                <h3 className="font-semibold text-sm text-foreground">Widget bulunamadı</h3>
