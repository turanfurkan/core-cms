...
/**
 * getPublicSettings - fetches all public settings from the Laravel backend using API key
 */
export async function getPublicSettings() {
  try {
    const backendUrl = process.env.BACKEND_API_URL || 'http://localhost:8000';
    const apiKey = process.env.BACKEND_API_KEY;
    const url = `${backendUrl}/api/settings/public`;
    const res = await fetch(url, {
      headers: {
        'Accept': 'application/json',
        'X-API-Key': apiKey || '',
      },
      next: { revalidate: 60 }, // Cache public settings for 60 seconds
    });
    if (!res.ok) return {};
    const data = await res.json();
    const settingsMap = {};
...
/**
 * getPublicNavigation - fetches navigation menu structure by key from public API
 */
export async function getPublicNavigation(key) {
  try {
    if (!key) return null;
    const backendUrl = process.env.BACKEND_API_URL || 'http://localhost:8000';
    const apiKey = process.env.BACKEND_API_KEY;
    const url = `${backendUrl}/api/navigations/${encodeURIComponent(key)}`;
    const res = await fetch(url, {
      headers: {
        'Accept': 'application/json',
        'X-API-Key': apiKey || '',
      },
      next: { revalidate: 60 }, // Cache navigation menu for 60 seconds
    });
    if (!res.ok) return null;
    const json = await res.json();
    return json.data;
  } catch (error) {
    console.error(`Error fetching public navigation for key ${key}:`, error);
    return null;
  }
}
