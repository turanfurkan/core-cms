...
/**
 * @mixin \App\Domains\Page\Models\Page
 */
class PageResource extends JsonResource
{
    ...
    protected function hydrateBlocks(array $blocks): array
    {
        ...
        if (!empty($globalBlockIds)) {
            $globalBlocks = \App\Domains\GlobalBlock\Models\GlobalBlock::whereIn('id', array_unique($globalBlockIds))
                ->get()
                ->keyBy('id');
        }
        ...
        if (!empty($categoryIds)) {
            $categories = \App\Domains\Category\Models\Category::with(['coverImage'])
        ...
        $posts = [];
        if (!empty($postIds)) {
            $posts = \App\Domains\Post\Models\Post::with(['coverImage', 'categories'])
        ...
        return array_map(function ($block) use ($races, $categories, $posts, $mediaItems) {
            if (($block['type'] ?? '') === 'collection_display') {
                ...
                if ($targetType === 'blog') {
                    $livePosts = \App\Domains\Post\Models\Post::published()
                        ->with(['coverImage', 'categories'])
                        ->orderBy('publish_date', 'desc')
                        ->orderBy('id', 'desc')
                        ->limit($limit)
                        ->get();
                    $block['hydrated_data'] = \App\Domains\Post\Http\Resources\PostResource::collection($livePosts)->toArray(request());
                } elseif ($targetType === 'races') {
                    $liveRaces = \App\Domains\Race\Models\Race::where('status', 'published')
                        ->with(['categories', 'coverImage'])
                        ->orderBy('order', 'asc')
                        ->orderBy('id', 'asc')
                        ->limit($limit)
                        ->get();
                    $block['hydrated_data'] = \App\Domains\Race\Http\Resources\RaceResource::collection($liveRaces)->toArray(request());
                }
                ...
                if ($sourceType === 'dynamic') {
                    $query = \App\Domains\Partner\Models\Partner::where('status', 'published');
                    if ($categorySlug) {
                    ...
                    $livePartners = $query->with(['categories', 'logo'])
                        ...
                        ->orderBy('id', 'desc')
                        ->get();
                    $block['hydrated_data'] = \App\Domains\Partner\Http\Resources\PartnerResource::collection($livePartners)->toArray(request());
                }
            } elseif (($block['type'] ?? '') === 'glassmorphic_grid') {
                $sourceType = $block['content']['source_type'] ?? 'category';
                $ids = $block['content']['entity_ids'] ?? [];
                ...
                if (is_array($ids)) {
                    foreach ($ids as $id) {
                        if ($sourceType === 'category' && isset($categories[$id])) {
                            ...
                            if (class_exists('\App\Domains\Post\Http\Resources\PostResource')) {
                                $resolved[] = (new \App\Domains\Post\Http\Resources\PostResource($posts[$id]))->toArray(request());
                            } else {
